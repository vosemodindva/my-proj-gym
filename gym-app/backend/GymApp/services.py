...
def assign_trainer_to_user(user, trainer_id, appointment_time_str):
    try:
        ...
        return Response({"detail": "Неверный формат времени"},
                        status=status.HTTP_400_BAD_REQUEST)

    if appointment_time < timezone.now():
        return Response({"detail": "Нельзя записаться в прошлое"},
                        status=status.HTTP_400_BAD_REQUEST)

    if not time(8, 0) <= appointment_time.time() <= time(18, 0):
        return Response({"detail": "Запись возможна только с 8:00 до 18:00"},
                        status=status.HTTP_400_BAD_REQUEST)
    ...
    if TrainerClient.objects.filter(
        ...
        return Response({"detail": "Это время уже занято"},
                        status=status.HTTP_400_BAD_REQUEST)

    try:
        trainer = Trainer.objects.get(id=trainer_id)
    except Trainer.DoesNotExist:
        return Response({"detail": "Тренер не найден"},
                        status=status.HTTP_404_NOT_FOUND)

    if TrainerClient.objects.filter(trainer=trainer, client=user).exists():
        return Response({"detail": "Вы уже записаны к этому тренеру"},
                        status=status.HTTP_400_BAD_REQUEST)

    TrainerClient.objects.create(trainer=trainer, client=user,
                                 appointment_time=appointment_time)

    return Response({"detail": "Запись прошла успешно"},
...
def unassign_trainer_from_user(trainer_id, user):
    ...
    deleted, _ = TrainerClient.objects.filter(trainer=trainer,
                                              client=user).delete()
    if deleted:
        return Response({"detail": "Вы отписались от тренера"},
                        status=status.HTTP_200_OK)
    else:
        return Response({"detail": "Вы не были записаны к этому тренеру"},
                        status=status.HTTP_400_BAD_REQUEST)
